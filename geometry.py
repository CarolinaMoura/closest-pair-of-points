...
def solve_closest_distance_nlog_divide_conquer(
    points: List[Point], inf: float = 1000000
) -> dict:

    # Untie x-coordinates by id
    points.sort()
    for ix, pt in enumerate(points):
        pt.id = ix

    # Pre-sort by y to reduce complexity by log
    points_sorted_by_y = sorted(points, key=lambda x: x.y)
    points.sort()

    # All the comparisons we do along the way while we find the closest distance
    comparison_lines = []
    ...
    def divide_and_conquer(points: List[Point], by_y: List[Point]) -> float:
        ...
        dist_left = divide_and_conquer(
            points[:mid], [pt for pt in by_y if pt.id < mid_x]
        )
        dist_right = divide_and_conquer(
            points[mid:], [pt for pt in by_y if pt.id >= mid_x]
        )
                    ...
                    comparison_lines.append(
                        (
                            ...
                            (
                                pt.x,
                                pt.y,
                                within_distance[ptr].x,
                                within_distance[ptr].y,
                            ),
                        )
                    )
                ptr += 1
...
def solve_closest_distance_nlog_line_sweep(
    points: List[Point], inf: float = 1000000
) -> dict:
    points.sort()
    s = SortedSet()
